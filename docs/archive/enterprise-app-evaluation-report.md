...
本次评测在项目目录外部全新的文件夹 `<工作区>/gaia-enterprise-app` 中，完整遵循 Gaia QuickStart 引导和技术文档，从零搭建了一个真实的**企业级电商订单与退款智控应用**。
...
* ✅ **YAML 配置与 Profile 的开发期可见性**：
  * 业务构建者在配置 `gaia.yaml` 时，容易混淆 `profile: mock` 与 `runtime.environment: mock`。
  * Dev Console 概览和配置页显示当前服务 Profile 与 Runtime Environment；CLI 和 API 错误继续明确报告环境不匹配。业务示例和试跑体验放在 Quick Start、Dev Doc 与独立 Showcase，不进入日常 Console。
...
* **简洁高效的 `GaiaAppBuilder` 组装**：
  * 在 `app.py` 中，开发者无需手动搭建复杂的 LangGraph 图结构或状态机。通过 `.scenarios(...)`、`.tools(...)`、`.model(...)` 和 `.retrieval(...)` 在一处完成装配，Gaia 自动绑定 `PersistentRuntimeEngine`、HTTP API 和 SSE；低层 `ApiDependencies.from_scenarios()` 仅保留给框架扩展和兼容代码。
* **高水准的幂等与防重机制**：
...
* ✅ **Prompt 与 RAG 资产路径已经脱离 CWD**：
  * `prompt.root` 和 `rag.root` 相对于 `gaia.yaml` 所在目录解析，Pytest、CLI 与服务进程可以从不同工作目录启动。
* ℹ️ **`GaiaApplication` 实例生命周期不可重入**：
  ...
  * 生成项目默认提供 `create_application()`，每个测试创建独立应用和数据库生命周期，不再要求业务方自行发现这一约束。
* ✅ **配置文件路径入口已经收敛**：
  * 使用 `--config`、`GAIA_CONFIG_PATH` 或默认 `gaia.yaml`；旧 `GAIA_CONFIG` 只保留弃用迁移提示。
