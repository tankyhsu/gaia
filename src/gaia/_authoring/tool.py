"""Application authoring helpers for declaring read and write tools."""
...
class FunctionToolSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if self.definition.kind == ToolKind.WRITE:
            strategy = self.definition.recovery_strategy
            if strategy == WriteRecoveryStrategy.RECONCILABLE and self.reconcile is None:
                raise ValueError("write tool requires an explicit reconcile handler")
            if self.reconcile is not None and not inspect.iscoroutinefunction(self.reconcile):
                raise TypeError("write tool reconcile handler must be async")
            if (
                strategy == WriteRecoveryStrategy.AT_MOST_ONCE_MANUAL
                and self.reconcile is not None
            ):
                raise ValueError("at-most-once write tool cannot declare a reconcile handler")
...
def write_tool(
    ...
    *,
    ...
    required_roles: tuple[str, ...] = (),
    reconcile: ReconcileHandler | None = None,
    recovery_strategy: WriteRecoveryStrategy = WriteRecoveryStrategy.RECONCILABLE,
    version: str = "1.0.0",
    timeout_seconds: int = 10,
    max_retries: int = 0,
    allowed_environments: tuple[RunMode, ...] = (RunMode.MOCK,),
) -> Callable[[HandlerType], HandlerType]:
    """Declare a write function with an explicit crash-recovery contract."""
    ...
    return _tool_decorator(
        ToolDefinition(
            ...
            idempotent=recovery_strategy == WriteRecoveryStrategy.IDEMPOTENT,
